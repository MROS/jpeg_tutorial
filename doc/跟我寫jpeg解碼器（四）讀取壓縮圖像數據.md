...
其實很簡單，每個 block 都是 8 * 8 ，最左上角的數值就是直流變量，要使用直流霍夫曼表來解碼；而餘下的 63 個數值是交流變量，使用交流霍夫曼表來解碼。
...
1. 讀取直流變量：
   - 不斷從數據流中讀取一個 bit ，直到可以對上直流霍夫曼表中的一個碼字，取出的對應信源編碼代表的是接下來還要讀入幾位，假如是 n 就繼續讀取 n bits ，以下表解碼後，就是直流變量。
2. 讀取交流變量
...
| -(2^n -1),……-2^(n - 1),2^(n - 1),……2^n - 1 | n <= 15 | 所有長度爲 n 的二進位           |
...
我們剛剛讀取到的直流變量，其實只是前一個 block 中的直流變量跟這一個 block 的直流變量的差而已。
...
[這份資料](./reference/JPEG图像解码方案.pdf)的第 3 頁結尾開始看，有一個很詳細的例子，就不再複述了。
